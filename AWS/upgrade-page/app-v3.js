...
function loadMarketingContent() {
  try {
    const raw = document.getElementById("planMarketingData")?.textContent || "{}";
    const parsed = JSON.parse(raw);
    return {
      descriptions: parsed?.descriptions || {},
      commonFeatures: Array.isArray(parsed?.commonFeatures) ? parsed.commonFeatures : []
    };
  } catch (error) {
    console.error("Failed to parse local marketing content:", error);
    return {
      descriptions: {},
      commonFeatures: []
    };
...
const MARKETING_CONTENT = loadMarketingContent();
...
function getPlanDescription(plan) {
  return MARKETING_CONTENT.descriptions?.[plan.planCode] || plan.description || "";
}

function getFeaturesIntro(plan) {
...
function getDisplayedFeatures(plan) {
  if (plan.planCode === "free") {
    return MARKETING_CONTENT.commonFeatures || [];
  }

  return (plan.features || []).filter((feature) => feature && feature.label);
}
...
function renderPlans(plans) {
  ...
  plans.forEach((plan) => {
    ...
    const featuresIntro = fragment.querySelector(".plan-card__features-intro");
    ...
    const introCopy = getFeaturesIntro(plan);
    if (introCopy) {
      featuresIntro.hidden = false;
      featuresIntro.textContent = introCopy;
    }

    getDisplayedFeatures(plan).forEach((feature) => {
