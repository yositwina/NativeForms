...
export default class NativeFormsPrefillActions extends LightningElement {
    ...
    get valueSourceHelpText() {
        return 'Choose where the value comes from: URL, verified email, form field, fixed value, or earlier alias.';
    }
    ...
    get conditionRows() {
        return (this.draftAction?.conditions || []).map((row, index) => ({
            ...
            isValueSourceVerifiedEmail: row.valueSource === 'verifiedEmail',
    ...
    async handleConditionChange(event) {
        ...
        if (name === 'valueSource') {
            ...
            if (value === 'verifiedEmail') {
                conditions[rowIndex].valueText = 'email';
            }
        }
        this.draftAction = {
            ...this.draftAction,
    ...
    buildConditionValueExpression(item) {
        if (item?.valueSource === 'verifiedEmail') {
            return '{params.email}';
        }
    ...
    getConditionValuePlaceholder(valueSource) {
        ...
        if (valueSource === 'verifiedEmail') {
            return 'Email entered in User Verification';
        }
        if (valueSource === 'alias') {
