...
import startConnectedOrg from '@salesforce/apex/NativeFormsConnectedOrgController.startConnect';
import disconnectConnectedOrg from '@salesforce/apex/NativeFormsConnectedOrgController.disconnectOrg';
...
export default class NativeFormsConnect extends NavigationMixin(LightningElement) {
    ...
    connectedOrgView;
    connectedOrgs = [];
    connectedOrgLoginBaseUrl = 'https://login.salesforce.com';
    connectedOrgError = '';
    connectedOrgMessage = '';
    isLoadingConnectedOrgs = false;
    isConnectingAdditionalOrg = false;
    isDisconnectingConnectedOrg = false;
    ...
    async loadSetupContext() {
        try {
            ...
            if (this.showConnectedOrgSection) {
                await this.loadConnectedOrgs();
            }
        } catch (error) {
    ...
    get showConnectedOrgSection() {
        return this.tenantSetupComplete && this.isConnected;
    }

    get connectedOrgLoginOptions() {
        return [
            { label: 'Production / Developer Edition', value: 'https://login.salesforce.com' },
            { label: 'Sandbox / Playground', value: 'https://test.salesforce.com' }
        ];
    }

    get hasConnectedOrgs() {
        return Array.isArray(this.connectedOrgs) && this.connectedOrgs.length > 0;
    }

    get connectedOrgActionDisabled() {
        return this.isLoadingConnectedOrgs || this.isConnectingAdditionalOrg || this.isDisconnectingConnectedOrg;
    }
    ...
    async loadConnectedOrgs() {
        if (this.isLoadingConnectedOrgs) {
            return;
        }

        this.isLoadingConnectedOrgs = true;
        this.connectedOrgError = '';
        try {
            const view = await getConnectedImportView();
            this.connectedOrgView = view || null;
            if (view?.success !== true) {
                this.connectedOrgError = view?.errorMessage || 'Connected orgs are not available right now.';
                this.connectedOrgs = [];
                return;
            }
            this.connectedOrgs = (view.orgs || [])
                .filter((item) => item.orgId && item.orgId !== this.orgId)
                .map((item) => ({
                    ...item,
                    key: item.orgId,
                    title: item.orgName ? `${item.orgName} (${item.orgId})` : item.orgId,
                    detail: `${item.orgType || 'org'} - ${item.status || 'active'}`
                }));
        } catch (error) {
            this.connectedOrgError = this.formatError(error);
            this.connectedOrgs = [];
        } finally {
            this.isLoadingConnectedOrgs = false;
        }
    }

    handleConnectedOrgLoginChange(event) {
        this.connectedOrgLoginBaseUrl = event.detail.value || 'https://login.salesforce.com';
    }

    async handleConnectAdditionalOrg() {
        this.connectedOrgError = '';
        this.connectedOrgMessage = '';
        this.isConnectingAdditionalOrg = true;

        try {
            const result = await startConnectedOrg({
                loginBaseUrl: this.connectedOrgLoginBaseUrl,
                returnUrl: window.location.href
            });
            if (result?.success !== true || !result.connectUrl) {
                throw new Error(result?.errorMessage || 'TwinaForms could not prepare the connected org authorization.');
            }
            this.connectedOrgMessage = 'Finish authentication in the Salesforce window, then return here and refresh connected orgs.';
            this.launchOauthWindow(result.connectUrl);
        } catch (error) {
            this.connectedOrgError = this.formatError(error);
            this.dispatchEvent(new ShowToastEvent({
                title: 'Could not connect org',
                message: this.connectedOrgError,
                variant: 'error',
                mode: 'sticky'
            }));
        } finally {
            this.isConnectingAdditionalOrg = false;
        }
    }

    async handleRefreshConnectedOrgs() {
        await this.loadConnectedOrgs();
    }

    async handleDisconnectConnectedOrg(event) {
        const targetOrgId = event.currentTarget?.dataset?.orgId;
        if (!targetOrgId || this.connectedOrgActionDisabled) {
            return;
        }

        this.isDisconnectingConnectedOrg = true;
        this.connectedOrgError = '';
        this.connectedOrgMessage = '';
        try {
            await disconnectConnectedOrg({ targetOrgId });
            this.connectedOrgMessage = 'Connected org removed.';
            await this.loadConnectedOrgs();
        } catch (error) {
            this.connectedOrgError = this.formatError(error);
        } finally {
            this.isDisconnectingConnectedOrg = false;
        }
    }
