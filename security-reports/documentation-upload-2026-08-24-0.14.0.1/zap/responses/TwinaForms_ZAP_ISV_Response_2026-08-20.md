...
Classification: Not applicable to current trust boundary / informational browser-header note.
...
Generated customer forms can include optional browser features such as CAPTCHA, customer styling, external image resources, and customer-approved custom JavaScript. The CSP header finding is therefore evaluated as browser-header guidance for the generated public page, not as evidence of exposed Salesforce data, tokens, credentials, tenant secrets, or authentication bypass. Runtime prefill and submit operations are still validated server-side with the published form token and configured form policies.

### Sub Resource Integrity Attribute Missing

Classification: Not applicable to current trust boundary / informational browser-resource note.

The published form can include external browser resources only for supported client-side form features. These resources are not used for authentication, authorization, tenant trust, Salesforce API access, or server-side policy enforcement.

Salesforce credentials, OAuth tokens, tenant bearer secrets, AWS secrets, and package signing secrets are never exposed to these browser resources. Server-side operations remain protected by TwinaForms runtime validation, publish-token checks, form-id checks, and configured tenant policies.
...
Classification: AWS-managed HTTPS endpoint behavior / informational browser-header note.

The Lambda Function URL is an AWS-managed HTTPS endpoint. Salesforce and browser traffic use HTTPS. The missing HSTS header is reported on the AWS-managed Lambda URL domain and is not evidence of plaintext transport.
...
The endpoint is served over HTTPS. The missing HSTS header is reported on the AWS-managed Lambda URL domain and is not evidence of plaintext transport.
...
The endpoint is HTTPS-only from the Salesforce package perspective. The missing HSTS header is reported on the AWS-managed Lambda URL domain and is not evidence of plaintext transport.
...
The missing HSTS header is reported on the AWS-managed Lambda Function URL. The admin web console itself is hosted on `admin.twinaforms.com` and uses Cognito Hosted UI login. This finding does not show plaintext transport, exposed credentials, or authentication bypass.
...
The portable snapshot routes are served over the AWS-managed HTTPS Lambda Function URL used by the Salesforce backend. The missing HSTS header is reported on that AWS-managed domain and is not evidence of plaintext transport or authorization bypass.

## Reviewer Disposition

The ZAP findings are expected public-form behavior, AWS-managed endpoint header behavior, or informational browser-resource notes. The scans confirm that protected tenant/backend/admin/import-export routes reject unauthenticated requests and that the public prefill/submit endpoints enforce publish-token and form-policy validation.
