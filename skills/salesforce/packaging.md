...
- Lightning app branding rule: keep the packaged `TwinaForms` app brand pointed at the packaged `TwinaForms_App_Logo` `ContentAsset`, so clean installs show the TwinaForms icon in App Launcher rather than an initial-based fallback tile.
...
- Element type metadata parity rule: before every package version, compare all element types that Apex/LWC can create or import against `force-app/main/default/objects/NF_Form_Element__c/fields/Element_Type__c.field-meta.xml`. New saved element types must be present in the picklist metadata before packaging. Keep `Element_Type__c` unrestricted because it is an internal package enum; a restricted subscriber picklist can reject new package element types after upgrade even when the Designer UI and Apex code support them. Virtual Designer-only elements such as submit button and User Verification controls do not need picklist values unless they are saved as `NF_Form_Element__c` records. Treat a mismatch or a restricted `Element_Type__c` metadata file as a release blocker.
- Portal Button packaging rule: `button` is a persisted Pro element gated by `enableProButtonElements`, stored through `Label__c` and `Config_JSON__c` without a field key. Keep its picklist value in unrestricted `Element_Type__c` and verify publish/runtime coverage before a package version.
- Country / State / City packaging rule: `location` is a persisted Pro field gated by `enableProLocationFields`, stored with a normal field key and grouped config in `Config_JSON__c`. Keep its picklist value in unrestricted `Element_Type__c`; V1 supports only top-level location fields, not Records List children.
...
- User Verification naming rule: customer-facing copy should say `User Verification` and `verification number`, not `Secret Code`, `secret code`, or `code` when referring to the customer-entered verification value. Avoid fixed English-only preview/runtime copy such as `Locked Until Verified` or `Code step appears after the user presses Enter`. Public-form button labels should be configurable when they are part of the customer experience. Package-visible API-name changes after a promoted managed release require a deliberate migration/deletion plan.
- Managed component deletion rule: a promoted managed 2GP package cannot remove previously packaged Apex classes, REST annotations, custom fields, or email templates unless Salesforce enables metadata deletion access through Partner Community. The `0.10.0.1` User Verification beta keeps old `Secret Code` API-name components as compatibility metadata while adding the new User Verification components.
...
- Current latest released install link is `https://login.salesforce.com/packaging/installPackage.apexp?p0=04tgL000000GWbpQAG` for `TwinaForms@0.9.0-8` / version `0.9.0.8`. It includes Merged Document, locked Lookup runtime behavior after prefill, Layout-to-Form picklist source-property preservation, textarea character-limit UX/publish handling, the Pro Button element, Country / State / City autocomplete, updated User Verification and Submission PDF/signature UX, scanned-file-upload support, App Launcher branding, 69 KB embedded image guards for publish stability, and the published 1-5 Star Rating label fix; code coverage was `83%`. Current latest beta test link is `https://test.salesforce.com/packaging/installPackage.apexp?p0=04tgL000000GZ6fQAG` for `TwinaForms@0.10.0-1` / version `0.10.0.1`; code coverage was not calculated for this beta package version. Update this skill and `AWS/documentation/Technical and specs/Salesforce_Packaging_Namespace_Audit_V1.md` every time a new package version is created or promoted.
- DevHub tenant safety rule: do not delete DynamoDB tenant records for DevHub/test orgs as a packaging reset step. For TwinaForms DevHub (`orgId=00DgL00000Q8RC9UAN`, alias `TwinaFormsDevHub`) preserve the `NativeFormsTenants` record and update plan/feature flags in place when testing Pro features.
