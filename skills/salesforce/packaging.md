...
- Current latest released install link is `https://login.salesforce.com/packaging/installPackage.apexp?p0=04tgL000000GWbpQAG` for `TwinaForms@0.9.0-8` / version `0.9.0.8`. It includes Merged Document, locked Lookup runtime behavior after prefill, Layout-to-Form picklist source-property preservation, textarea character-limit UX/publish handling, the Pro Button element, Country / State / City autocomplete, updated User Verification and Submission PDF/signature UX, scanned-file-upload support, App Launcher branding, 69 KB embedded image guards for publish stability, and the published 1-5 Star Rating label fix; code coverage was `83%`. Current latest beta test link is `https://test.salesforce.com/packaging/installPackage.apexp?p0=04tgL000000GWTlQAO` for `TwinaForms@0.9.0-7` / version `0.9.0.7`; code coverage was not calculated for this beta package version. Update this skill and `AWS/documentation/Technical and specs/Salesforce_Packaging_Namespace_Audit_V1.md` every time a new package version is created or promoted.
