...
- Permission-set management location rule: manage `TwinaForms User` / `TwinaForms Admin` assignment from the `TwinaForms Connect` page, not `TwinaForms Home`, so setup/access troubleshooting stays in one place. When Connect grants or removes a `TwinaForms User` seat, it must also grant or remove the subscriber-created `TwinaForms Credentials` permission set for that same user.
...
- External Credential packaging rule: do not rely on `externalCredentialPrincipalAccesses` inside packaged permission-set metadata for install success. Keep the subscriber setup flow/documentation explicit about creating `TwinaForms Credentials` with API name `TwinaForms_Credentials` and the manual External Credential Principal Access entries. After that setup exists, Connect should manage assignment of `TwinaForms Credentials` together with `TwinaForms User`; code may fall back to a permission set that already grants both known TwinaForms external credential principals.
...
- Current latest beta install link is `https://login.salesforce.com/packaging/installPackage.apexp?p0=04tgL000000ERGTQA4`. Update this skill and `AWS/documentation/Technical and specs/Salesforce_Packaging_Namespace_Audit_V1.md` every time a new package version is created.
