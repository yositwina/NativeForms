...
- Secret-code Designer UX is hybrid: Form Settings owns only the enable/disable checkbox and a short hint, while the visible Secret Code canvas block is selectable and exposes messages, button labels, retry/session rules, and Contact/email match details in the right-side properties panel. Treat it as a system block, not a normal input field or submitted Salesforce value.
...
- Create form from Salesforce page layout is a Pro productivity feature gated by `enableProPageLayoutClone`: admins choose a language and object, AWS uses the stored Salesforce OAuth connection to read the assigned Salesforce page layout via UI API with `Accept-Language`, and TwinaForms creates a draft form from supported layout sections/fields using Salesforce translated labels when available. Hebrew imports force RTL on the created form. If Salesforce returns base labels for a non-English import, create the form and warn the admin to check Translation Workbench. The form adds prefill by `recordId` and configures submit to update-or-create the same object. V1 ignores Salesforce ownership/audit/system fields, skips unsupported fields, and can include create-only lookup/reference fields as locked-when-prefilled, create-only submit mappings so updates omit them while fallback creates include them. After import, show an acknowledgement modal with imported/create-only/skipped details and field-level guidance. Secure update flows always use Salesforce Contact/email secret-code verification, regardless of the target object. If the visitor email is not a Salesforce Contact, the visitor cannot edit through secret-code mode; V1 does not attempt generic arbitrary-object identity verification. V1 does not retrieve arbitrary unassigned layout files by Metadata API.
